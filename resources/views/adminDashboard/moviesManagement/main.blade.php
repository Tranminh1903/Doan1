...
@push('scripts')
<script>
/* Đặt hàm lên window để onclick inline có thể gọi được */
window.openEditModal = function(promotion) {
  console.log('Promotion:', promotion);

  const form = document.querySelector('#editPromotionForm');
  if (!form) {
    console.error('Không tìm thấy form #editPromotionForm');
    return;
  }

  form.action = `/adminDashboard/promotion/update/${promotion.id}`;

  // Chuyển datetime server -> format cho <input type="datetime-local">
  const toDatetimeLocal = (dateStr) => {
    if (!dateStr) return '';
    const d = new Date(dateStr);
    if (Number.isNaN(d.getTime())) return '';
    // toISOString() trả "YYYY-MM-DDTHH:MM:SS.sssZ" -> slice 0..16 để lấy "YYYY-MM-DDTHH:MM"
    return d.toISOString().slice(0, 16);
  };

  // Gán giá trị vào form (bảo vệ null/undefined)
  document.querySelector('#edit_code').value = promotion.code ?? '';
  document.querySelector('#edit_type').value = promotion.type ?? 'fixed';
  ...
  document.querySelector('#edit_start_date').value = toDatetimeLocal(promotion.start_date);
  document.querySelector('#edit_end_date').value = toDatetimeLocal(promotion.end_date);
  ...
  if (!modalEl) {
    console.error('Không tìm thấy modal #editPromotionModal');
    return;
  }
